...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyStructuredResultV2> => {
  try {
    loggerService.trace("getGoogleAccounts called", { event }, "getGoogleAccounts handler");

    const response = await calendarController.getGoogleAccounts(event);

    return response;
  } catch (error: unknown) {
    // We should never get here, as Controller classes should never throw, but if for some reason we do, we need to log it
    loggerService.error("Catastrophic error in getAccounts handler", { error, event }, "getGoogleAccounts handler");
