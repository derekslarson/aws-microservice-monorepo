...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyStructuredResultV2> => {
  try {
    loggerService.trace("createMeetingMessage called", { event }, "createMeetingMessage handler");

    const response = await messageController.createMeetingMessage(event);

    return response;
  } catch (error: unknown) {
    // We should never get here, as Controller classes should never throw, but if for some reason we do, we need to log it
    loggerService.error("Catastrophic error in createMeetingMessage handler", { error, event }, "createMeetingMessage handler");
