...
export class ValidationService implements ValidationServiceInterface {
  ...
  constructor(
  @inject(TYPES.ClassTransformerFactory)
    classTransformerFactory: ClassTransformerFactory,
    @inject(TYPES.ClassValidatorFactory)
    classValidatorFactory: ClassValidatorFactory,
...
export interface ValidationServiceInterface {
  validate<T>(dtoConstructor: { new (): T }, jsonBody?: string): Promise<T>;
}
