...
@injectable()
export class SnsEventController implements SnsEventControllerInterface {
  constructor(
  ...
  public async handleSnsEvent(event: SNSEvent): Promise<void> {
    try {
      this.loggerService.trace("handleSnsEvent called", { event }, this.constructor.name);
      ...
      this.loggerService.error("Error in handleSnsEvent", { error, event }, this.constructor.name);
    }
  }
  ...
  private prepareRecordsForProcessorServices(record: SNSEventRecord): SnsProcessorServiceRecord {
    try {
      this.loggerService.trace("prepareRecordsForProcessorServices called", { record }, this.constructor.name);

      const { Message, TopicArn } = record.Sns;
...
export interface SnsEventControllerInterface {
