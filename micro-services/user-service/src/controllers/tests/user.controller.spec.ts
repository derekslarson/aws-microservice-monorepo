...
describe("UserController", () => {
  ...
  beforeEach(() => {
    ...
    userController = new UserController(validationService, loggerService, userService);
  });
  ...
  describe("getTeamsByUserId", () => {
    ...
    describe("under normal conditions", () => {
      beforeEach(() => {
        ...
        userService.getTeamsByUserId.and.returnValue(Promise.resolve(mockServiceResponse));
      ...
      it("calls this.getUserIdFromRequestWithJwt with the correct params", async () => {
        await userController.getTeamsByUserId(mockRequest);
      ...
      it("calls userService.getTeamsByUserId with the correct params", async () => {
        await userController.getTeamsByUserId(mockRequest);

        expect(userService.getTeamsByUserId).toHaveBeenCalledTimes(1);
        expect(userService.getTeamsByUserId).toHaveBeenCalledWith(mockUserId);
      });
      ...
      it("returns the response of this.generateSuccessResponse", async () => {
        const response = await userController.getTeamsByUserId(mockRequest);
    ...
    describe("under error conditions", () => {
      ...
      describe("when an error is thrown", () => {
        ...
        it("calls loggerService.error with the correct params", async () => {
          ...
          expect(loggerService.error).toHaveBeenCalledWith("Error in getTeamsByUserId", { error: mockError, request: mockRequest }, userController.constructor.name);
        });
