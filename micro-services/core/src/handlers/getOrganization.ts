...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyStructuredResultV2> => {
  try {
    loggerService.trace("getOrganization called", { event }, "getOrganization handler");

    const response = await organizationController.getOrganization(event);

    return response;
  } catch (error: unknown) {
    // We should never get here, as Controller classes should never throw, but if for some reason we do, we need to log it
    loggerService.error("Catastrophic error in getOrganization handler", { error, event }, "getOrganization handler");
