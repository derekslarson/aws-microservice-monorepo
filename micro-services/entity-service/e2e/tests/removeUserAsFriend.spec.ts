...
import { createRandomUser, createConversationUserRelationship, createFriendConversation, getConversation, getConversationUserRelationship } from "../util";
...
describe("DELETE /users/{userId}/friends/{friendId} (Remove User as Friend)", () => {
  ...
  describe("under normal conditions", () => {
    ...
    it("returns a valid response", async () => {
      ...
      try {
        const { status, data } = await axios.delete<{ message: string; }>(`${baseUrl}/users/${userId}/friends/${otherUser.id}`, { headers });
