...
describe("DELETE /meetings/{meetingId}/users/{userId} (Remove User from Meeting)", () => {
  ...
  describe("under normal conditions", () => {
    ...
    beforeEach(async () => {
      ...
      await createConversationUserRelationship({ userId, conversationId: meeting.id, role: Role.Admin });
    });
    ...
    it("returns a valid response", async () => {
      ...
      try {
        const { status, data } = await axios.delete<{ message: string; }>(`${baseUrl}/meetings/${meeting.id}/users/${otherUser.id}`, { headers });
    ...
    it("deletes the necessary ConversationUserRelationship entity", async () => {
      ...
      try {
        await axios.delete<{ message: string; }>(`${baseUrl}/meetings/${meeting.id}/users/${otherUser.id}`, { headers });
  ...
  describe("under error conditions", () => {
    ...
    describe("when an id of a meeting the user is not an admin of is passed in", () => {
      ...
      beforeEach(async () => {
        ...
        await createConversationUserRelationship({ userId, conversationId: meetingTwo.id, role: Role.User });
      });
