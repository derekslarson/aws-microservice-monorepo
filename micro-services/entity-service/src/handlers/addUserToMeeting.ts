...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyStructuredResultV2> => {
  try {
    loggerService.trace("addUserToMeeting called", { event }, "addUserToMeeting handler");

    const response = await meetingController.addUserToMeeting(event);

    return response;
  } catch (error: unknown) {
    // We should never get here, as Controller classes should never throw, but if for some reason we do, we need to log it
    loggerService.error("Catastrophic error in addUserToMeeting handler", { error, event }, "addUserToMeeting handler");
