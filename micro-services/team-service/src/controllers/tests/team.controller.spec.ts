  ...
  describe("getUsersByTeamId", () => {
    ...
    describe("under normal conditions", () => {
      beforeEach(() => {
        ...
        teamService.getUsersByTeamId.and.returnValue(Promise.resolve(mockServiceResponse));
        spyOn(teamController, "generateSuccessResponse").and.returnValue(mockSuccessResponse);
      });

      it("calls this.getUserIdFromRequestWithJwt with the correct params", async () => {
        await teamController.getUsersByTeamId(mockRequest);
      ...
      it("calls teamService.getUsersByTeamId with the correct params", async () => {
        await teamController.getUsersByTeamId(mockRequest);

        expect(teamService.getUsersByTeamId).toHaveBeenCalledTimes(1);
        expect(teamService.getUsersByTeamId).toHaveBeenCalledWith(mockTeamId);
      });
      ...
      it("returns the response of this.generateSuccessResponse", async () => {
        const response = await teamController.getUsersByTeamId(mockRequest);
    ...
    describe("under error conditions", () => {
      ...
      describe("when an error is thrown", () => {
        ...
        it("calls loggerService.error with the correct params", async () => {
          ...
          expect(loggerService.error).toHaveBeenCalledTimes(1);
          expect(loggerService.error).toHaveBeenCalledWith("Error in getUsersByTeamId", { error: mockError, request: mockRequest }, teamController.constructor.name);
        });
