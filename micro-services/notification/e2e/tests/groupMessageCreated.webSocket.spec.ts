...
import { GroupMessageCreatedWebSocketMessage } from "../../src/models/websocket-messages/groupMessageCreated.websocket.message";
import { UserId } from "../../../core/src/types/userId.type";

describe("Group Message Created (WebSocket Event)", () => {
  ...
  const connections: Record<string, { messages: GroupMessageCreatedWebSocketMessage[], instance: null | WebSocket }> = {
    userOneA: {
      messages: [],
      instance: null,
    },
  ...
  beforeAll(async () => {
    ...
    await Promise.all(Object.keys(connections).map((connectionName) => new Promise((resolve, reject) => {
      ...
      instance.on("message", (event) => {
        ...
        if (message.event === WebSocketEvent.GroupMessageCreated) {
