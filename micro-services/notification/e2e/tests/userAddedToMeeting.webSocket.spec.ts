...
import { UserAddedToMeetingWebSocketMessage } from "../../src/models/websocket-messages/userAddedToMeeting.websocket.message";

describe("User Added To Meeting (WebSocket Event)", () => {
  ...
  const connections: Record<string, { messages: UserAddedToMeetingWebSocketMessage[], instance: null | WebSocket }> = {
    userOneA: {
      messages: [],
      instance: null,
    },
  ...
  beforeAll(async () => {
    ...
    await Promise.all(Object.keys(connections).map((connectionName) => new Promise((resolve, reject) => {
      ...
      instance.on("message", (event) => {
        ...
        if (message.event === WebSocketEvent.UserAddedToMeeting) {
