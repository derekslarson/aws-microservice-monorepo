...
import { MeetingMessageCreatedWebSocketMessage } from "../../src/models/websocket-messages/meetingMessageCreated.websocket.message";
import { UserId } from "../../../core/src/types/userId.type";

describe("Meeting Message Created (WebSocket Event)", () => {
  ...
  const connections: Record<string, { messages: MeetingMessageCreatedWebSocketMessage[], instance: null | WebSocket }> = {
    userOneA: {
      messages: [],
      instance: null,
    },
  ...
  beforeAll(async () => {
    ...
    await Promise.all(Object.keys(connections).map((connectionName) => new Promise((resolve, reject) => {
      ...
      instance.on("message", (event) => {
        ...
        if (message.event === WebSocketEvent.MeetingMessageCreated) {
