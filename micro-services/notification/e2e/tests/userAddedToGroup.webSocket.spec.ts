...
import { UserAddedToGroupWebSocketMessage } from "../../src/models/websocket-messages/userAddedToGroup.websocket.message";

describe("User Added To Group (WebSocket Event)", () => {
  ...
  const connections: Record<string, { messages: UserAddedToGroupWebSocketMessage[], instance: null | WebSocket }> = {
    userOneA: {
      messages: [],
      instance: null,
    },
  ...
  beforeAll(async () => {
    ...
    await Promise.all(Object.keys(connections).map((connectionName) => new Promise((resolve, reject) => {
      ...
      instance.on("message", (event) => {
        ...
        if (message.event === WebSocketEvent.UserAddedToGroup) {
